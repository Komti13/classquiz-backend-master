        ...
        <div class="span12">
            <div class="grid simple ">
                <div class="grid-title">
                    <h4>List <span class="semi-bold">Pack promotions</span></h4>
                    <div class="tools">
...
@section('js')
    ...
    <script type="text/javascript">
        var oTable3 = $('#example3').dataTable({
            processing: true,
            serverSide: true,
            ajax: "{{ route('pack-promotions.index') }}",
            columns: [
                {data: 'id'},
                {
                    data: null,
                    ...
                    render: function(data, type, row, meta) {
                        return `<a class="badge badge-primary" href="/packs/${row.pack.id}/edit">${row.pack.name}</a>`;
                    }
                },
                {
                    data: null,
                    ...
                    searchable: false,
                    render: function(data, type, row, meta) {
                        return `<a class="badge badge-primary" href="/levels/${row.pack.level.id}/edit">${row.pack.level.name}</a>`;
                    }
                },
                {data: 'start'},
                {data: 'end'},
                    ...
                    render: function(data, type, row, meta) {
                        return `<div class="btn-group">
                                    <a class="btn btn-mini btn-white"
                                       href="/pack-promotions/${row.id}/edit">Edit</a>
        ...
        $("div.toolbar").html('<div class="table-tools-actions"><a class="btn btn-primary" href="{{ route('pack-promotions.create') }}" style="margin-left:12px" id="test2">Add</a></div>');

        $(document).on('click', '.delete', function(e) {
            const id = $(this).data('id');
            if (confirm('Are you sure?')) {
                $.post('/pack-promotions/'+id, {_method: 'DELETE', _token: $('meta[name="csrf-token"]').attr('content')}, function(){
                    location.reload();
                });
            }
        });
