        ...
        <div class="span12">
            <div class="grid simple ">
                <div class="grid-title">
                    <h4>List <span class="semi-bold">Delegations</span></h4>
                    <div class="tools">
...
@section('js')
    ...
    <script type="text/javascript">
            ...
            columns: [
                ...
                {data: 'name'},
                {
                    ...
                    render: function(data, type, row, meta) {
                        return `<span class="label label-success">${row.governorate.name}</span>`;
                    }
                },
                {data: 'created_at'},
                {
                    ...
                    render: function(data, type, row, meta) {
                        return `<div class="btn-group">
                                    <a class="btn btn-mini btn-white"
                                       href="/delegations/${row.id}/edit">Edit</a>
        ...
        $("div.toolbar").html('<div class="table-tools-actions"><a class="btn btn-primary" href="{{ route('delegations.create') }}" style="margin-left:12px" id="test2">Add</a></div>');

        $(document).on('click', '.delete', function(e) {
            const id = $(this).data('id');
            if (confirm('Are you sure?')) {
                $.post('/delegations/'+id, {_method: 'DELETE', _token: $('meta[name="csrf-token"]').attr('content')}, function(){
                    location.reload();
                });
            }
        });
