@extends('admin.layout.layout',['title'=>'Countries'])
@section('css')
    <!-- BEGIN PLUGIN CSS -->
...
@section('js')
    ...
    <script type="text/javascript">
                    ...
                    render: function(data, type, row, meta) {
                        return `<div class="btn-group">
                                    <a class="btn btn-mini btn-white"
                                       href="/countries/${row.id}/edit">Edit</a>
        ...
        $("div.toolbar").html('<div class="table-tools-actions"><a class="btn btn-primary" href="{{ route('countries.create') }}" style="margin-left:12px" id="test2">Add</a></div>');

        $(document).on('click', '.delete', function(e) {
            const id = $(this).data('id');
            if (confirm('Are you sure?')) {
                $.post('/countries/'+id, {_method: 'DELETE', _token: $('meta[name="csrf-token"]').attr('content')}, function(){
                    location.reload();
                });
            }
        });
