        ...
        <div class="span12">
            <div class="grid simple ">
                <div class="grid-title">
                    <h4>List <span class="semi-bold">Chapter types</span></h4>
                    <div class="tools">
...
@section('js')
    ...
    <script type="text/javascript">
        var oTable3 = $('#example3').DataTable({
            processing: true,
            serverSide: true,
            ajax: "{{ route('chapter-types.index') }}",
            columns: [
                {data: 'id'},
                {data: 'name'},
                {data: 'order'},
                    ...
                    render: function(data, type, row, meta) {
                        return `<div class="btn-group">
                                    <a class="btn btn-mini btn-white"
                                       href="/chapter-types/${row.id}/edit">Edit</a>
        ...
        $("div.toolbar").html('<div class="table-tools-actions"><a class="btn btn-primary" href="{{ route('chapter-types.create') }}" style="margin-left:12px" id="test2">Add</a></div>');

        $(document).on('click', '.delete', function(e) {
            const id = $(this).data('id');
            if (confirm('Are you sure?')) {
                $.post('/chapter-types/'+id, {_method: 'DELETE', _token: $('meta[name="csrf-token"]').attr('content')}, function(){
                    location.reload();
                });
            }
        });
